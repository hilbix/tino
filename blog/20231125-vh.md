...
**Katastrophe**, denn wie immer wurde alles bei Apache auf maximalen Schaden gebürstet.  (Mit PHP-FPM wäre das nicht passiert!)
...
Und: `mpm_prefork` ist so das ziemlich dümmste, das man in meiner Situation gebrauchen kann.  Natürlich aber wird das gewählt.

> Der Apache stand aus gutem Grund auf `event`.  Die einzig brauchbare Alternative dazu ist `worker`.
>
> Aber klar doch, dass nur noch das einzig am wenigsten sinnvolle Modul mit PHP unterstützt wird.
>
> Ich habe es so satt.  Wird wirklich Zeit, das auf FPM umzustellen und diesen inzwischen total unbrauchbaren Apache langsam loszuwerden.
